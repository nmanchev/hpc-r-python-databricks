...
DATABRICKS_HTTPPATH <- "<YOUR_HTTPPATH_HERE>"
DATABRICKS_HOST <- "<YOUR_HOST_URL_HERE>"
...
server <- function(input, output, session) {
  ...
  user <- reactive({
    databricks_user(session, DATABRICKS_HOST)
  })
  
  output$display_name <- renderText({
    paste0("Hi, ", user()$displayName, "!")
  })
  ...
  filtered_data <- reactive({
    ...
    data <- tbl(
      pool_con(),
      in_catalog("posit_demo", "default", "lending_club")
    ) |>
  ...
  output$loan_details_table <- render_gt({
    ...
    data_transformed <-
      raw_data |>
      select(region, office_no, grade, sub_grade, loan_amnt, term, int_rate,
             emp_title, emp_length, annual_inc, loan_status,
             title, addr_state, out_prncp) |>
      mutate(loan_amnt = as.numeric(loan_amnt),
             annual_inc = as.numeric(annual_inc),
             out_prncp = as.numeric(out_prncp)) |>
      rename(Region = region, Office = office_no, State = addr_state, Grade = grade, `Sub Grade` = sub_grade,
    ...
    table_start |>
      opt_interactive(use_compact_mode = TRUE, use_highlight = TRUE, use_text_wrapping = FALSE, page_size_default = 10)
  })
